...
const FirebaseDB = {
  // Save game to Firebase instead of localStorage - FIXED
  saveGame: async function(gameCode, gameData) {
    try {
      // Clean the data - remove any functions or circular references
      const cleanData = {
        code: gameData.code,
        host: gameData.host,
        guest: gameData.guest,
        escrowWallet: gameData.escrowWallet,
        escrowSecretKey: gameData.escrowSecretKey, // Keep this for reconstruction
        hostDeposited: gameData.hostDeposited || false,
        guestDeposited: gameData.guestDeposited || false,
        gameStarted: gameData.gameStarted || false,
        createdAt: gameData.createdAt || Date.now(),
        lastUpdated: Date.now()
      };
      
      await database.ref(`games/${gameCode}`).set(cleanData);
